...
function render(data){
  ...
  const saved = localStorage.getItem(`target:${data.id}`);
  targetInput.value = saved ? fmt(Number(saved)) : "";
...
function renderHistory(history){
  ...
  box.innerHTML=`<div class="history-title">روند آخرین ثبت‌های قیمت</div><div class="bars">${prices.slice().reverse().map((p,i)=>`<div class="bar" style="height:${18+((p-min)/span)*72}%"><span>${fmt(p)} تومان</span></div>`).join("")}</div>`;
}

async function lookup(url){
  const res=await fetch(`${API_BASE}/api/price?url=${encodeURIComponent(url)}`,{headers:{Accept:"application/json"}});
...
async function poll(url){
  ...
  try{
    ...
    if(data.status === "ready"){
      hideStatus(); render(data); return;
    }
    status("⏳ لینک دریافت شد؛ قیمت در حال دریافت است. چند ثانیه دیگر دوباره بررسی می‌کنیم…");
    pollTimer=setTimeout(()=>poll(url),7000);
...
setInterval(()=>{
  if(!currentProduct)return;
  const target=Number(localStorage.getItem(`target:${currentProduct.id}`));
  const price=currentProduct.price/10;
