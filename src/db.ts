...
export async function getLatestUsdRateWithAge(env: Env): Promise<{ rate: number; ageSeconds: number } | null> {
  const row = await env.DB.prepare(
    `SELECT rate, (strftime('%s','now') - timestamp) AS age FROM usd_rates ORDER BY timestamp DESC LIMIT 1`
  ).first<{ rate: number; age: number }>();
  return row ? { rate: row.rate, ageSeconds: row.age } : null;
}
